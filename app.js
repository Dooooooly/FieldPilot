...
function addRegion() {
    var name = prompt('새 지역명 (영문):', 'newregion');
    if (name && name.trim()) {
        var region = name.trim().toLowerCase();
...
function renderPlaces(filtered) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += '<div class="addr">' + (p.address || '') + '</div>';
        html += '</div>';
